...
def reassign(args, block_split_dict, block_sims, assignment):
    num_model = len(block_split_dict)
    ...
    for i, center_block in enumerate(centers):
        for m, other_model_name in enumerate(block_split_dict):
...
def repartition(args, block_split_dict, block_sims, all_assignmemt):
    improved = False
    for m_id, model_name in enumerate(block_split_dict):
...
def init_partition(args):
    ...
    for model_name in block_split_dict:
        node_list = block_split_dict[model_name]
...
def get_all_sim(args):
    all_sim_dict = dict()
    comb = list(combinations(all_sim_dict, 2))
    comb += [(m, m) for m in all_sim_dict]

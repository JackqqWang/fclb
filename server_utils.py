...
def search_path(target_model, pool_set):
    target_model_layers = get_layers(target_model)
    search_path = []
    for each_pool in pool_set:
        for input_layers in each_pool:
            order_layers = sort_layers(input_layers)
    for i in len(target_model_layers):
        if i == 0:
            search_path.append(random.choicepool_set[i])
        else:
            search_path.append(match_best_layer(target_model_layers[i], pool_set[i]))
    return search_path

def match_best_layer(input1, pool):
    sim = 0
    for item in pool:
        current_score = compute_sim(input1, item)
        if current_score >= sim:
            sim = current_score
    return item
...
def block_completion(blocks, following_blocks):
    block_completion_results = blocks
    for remaining_block in following_blocks:
        blocks.append(remaining_block)
    return block_completion_results
...
def layer_feature(model_dict, dataset, client_model_info, args):
